...
public class InfisicalSecrets implements Serializable {
  ...
  public static List<SingleSecretResponse> getSecrets(InfisicalConfiguration configuration,
      InfisicalCredential credential, String secretPath, boolean includeImports, PrintStream logger) {
    ...
    try {
      ...
      String urlString = String.format(
          ...
          "/api/v3/secrets/raw", URLEncoder.encode(secretPath, "UTF-8"), configuration.getInfisicalProjectSlug(),
          configuration.getInfisicalEnvironmentSlug(), includeImports ? "true" : "false");
        ...
        if (includeImports && imports != null && !imports.isEmpty()) {
          ...
          imports.forEach(importedSecret -> {
            try {
              ...
              // We need to make sure the response.secrets only has one key with the same
